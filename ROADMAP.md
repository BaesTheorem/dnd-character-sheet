...
Settings → Integrations: Beyond20 live-roll bridge (Roll20 / Foundry VTT / Discord), Discord webhook, Foundry `dnd5e` actor export, and importers for D&D Beyond and character-sheet PDFs (MPMB + the official fillable sheet, read directly via DecompressionStream). Follow-ups if wanted:
...
- D&D Beyond fetch is paste-only in practice (no CORS on their character service). A proxy would fix it but would route character data through a third party — not worth it unasked. Their PDF export is not an option either: it carries no form fields and draws every value as vector outlines, so nothing is readable without OCR.
- Owlbear Rodeo was built and then **removed in 2.156.0**: an extension has to be reachable over the web, and browser storage is per-origin, so an embedded copy could never see the characters in someone's local file. Don't rebuild it without solving that first.
