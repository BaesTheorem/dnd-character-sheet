...
## Themes
Each character can carry its own **theme** — a named look chosen under **Settings → Character Settings → Theme** and saved with that character. Most themes are a swap of the sheet's CSS custom properties (and a few decorative touches); one (D&D Beyond) also rearranges the layout. Three ship today:
...
- **D&D Beyond** — a landscape sheet in the style of D&D Beyond: a dark, gold-accented frame where your abilities, saving throws, skills, senses, proficiencies, and combat stats wrap around a tabbed centre panel that switches between **Actions** (attacks + limited-use features), **Spells**, **Inventory**, **Features & Traits**, **Background**, **Notes**, **Backstory**, and **Extras** (companions). Unlike the other themes it changes the on-screen layout, not just the palette: a small engine relocates the Core cards into a `#ddb-*` scaffold and builds the centre tab bar (moving the real nodes, so all live calculation keeps working). It self-heals a file saved while active and restores the standard one-page-per-tab flow for **printing** (D&D Beyond prints page-by-page like the others). Has its own light and dark palette like every theme; the dark one matches D&D Beyond's dark mode.
