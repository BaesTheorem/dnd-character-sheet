...
A self-contained, **automated** D&D 5e (2014 rules) character sheet that runs in any web browser
with **no install, no server, and no Adobe**. It works offline, and a blank copy prints as a
perfectly good pen-and-paper sheet. The whole app (layout, styling, automation, and a full
sourcebook library) lives in one file: `index.html` (~14 MB, most of which is the baked reference data).
...
## iPhone & iPad (iOS)
On iOS, use the web version, which installs as a real offline app:
...
- **Senses & Defenses** compute into chips from your race, feats, features, and equipped magic items,
  and update as you (un)equip and (un)attune, so there are no stale hand-written lines.
...
also accepts homebrew **classes**. Everything you author gets **real mechanical support** on the
sheet rather than inert text, and each source is an independent book you can toggle on/off or share. All
...
**Loading and managing the books themselves, under Settings → Advanced → Source books:**
- **From a book file (no server).** On a sheet with no sourcebooks loaded, a "No sourcebooks loaded"
  ...
  loaded library, bundled in a .zip. That is the way to hand books to someone else.
...
- **Rules:** D&D 5e **2014** ruleset only. 2024 ("One D&D") books are filtered out of the book picker
  and blocked from loading on purpose, because this sheet is built for the 2014 rules.
